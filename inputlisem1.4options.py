...
class DEMderivatives(StaticModel):
    ...
    def initial(self):
        ...
        Ldd = lddcreate (DEMc-chanm*10-mainout*100, size, size, size, size)
        report(Ldd, LddName)
        Ldd_ = Ldd
...
class ChannelMaps(StaticModel):
    ...
    def initial(self):
        ...
        lddchan = lddcreate((DEM-mainout*100)*chanmask,1e20,1e20,1e20,1e20)
        cm = chanmask
        if doPruneBranch == 1:
            # delete isolated branches of 1 cell
            cm = ifthen(accuflux(lddchan,1) > 1,chanmask)*mask
            lddchan = lddcreate((DEM-mainout*100)*cm,1e20,1e20,1e20,1e20)
            cm = cover(cm, 0)*mask
            chanmask = cm
        ...
        if doUserOutlets == 0 :
            ...
            channelout = mainout_
            Ldd = readmap(LddName)
            ws = catchment(Ldd, nominal(mainout_))
        ...
        ### VJ 210522
        baseflow=mainout*chBaseflowmap
...
if __name__ == "__main__":
   ...
   #    sys.exit()
    ...
    mainout_=zero_
    if doUserOutlets == 1:
       mainout_ = scalar(readmap(outletsbaseName))
