...
LddName= workingDir+'ldd.map'             # Local Drain Direction for surface runoff
...
shadeName= workingDir+'shade.map'         # shaded relief map, not use din lisem

# infrastructure
roadwidthName= workingDir+'roadwidt.map'   # road width (m)
hardsurfName= workingDir+'hardsurf.map'    # impermeable surfaces (0 or 1)
housecovName= workingDir+'housecover.map'  # house cover fraction
roofstoreName= workingDir+'roofstore.map'  # roof interception (mm) \
raindrumsizeName= workingDir+'raindrum.map'# raindrum size (m3)
drumstoreName= workingDir+'drumstore.map'  # locations of rainwater harvesting in drums (0/1)

# vegetation maps
coverName= workingDir+'per.map'           # cover fraction (-)
laiName= workingDir+'lai.map'             # leaf area index (m2/m2) for interception storage
smaxName= workingDir+'smax.map'           # user defined canopy storage
cropheightName= workingDir+'ch.map'       # plant height in m, for erosion, not used
grasswidName= workingDir+'grasswid.map'   # width of grass strips for infiltration
LitterName= workingDir+'litter.map'       # fraction of litter under tree vegetation.

# Green and Ampt infiltration maps, numbers 1 and 2 are added to the names in the script
ksatName= workingDir+'ksat'                # sat hydraulic conductivity (mm/h)
poreName= workingDir+'thetas'              # porosity (-)
thetaiName= workingDir+'thetai'            # initial moisture content (-)
psiName= workingDir+'psi'                  # suction unsat zone (cm)
...
ksatcrustName= workingDir+'ksatcrust.map'  # ksat of crusted areas (mm/h)
porecompName= workingDir+'porecomp.map'    # Porosity of compacted areas (-)
porecrustName= workingDir+'porecomp.map'   # Porosity of crusted areas (-)

# surface maps
rrName= workingDir+'rr.map'                # surface roughness (cm)
mannName= workingDir+'n.map'               # mannings n ()
stoneName= workingDir+'stonefrc.map'       # stone fraction on surface (-)
...
D90Name= workingDir+'d90.map'			     # 90 quantile of texture for bedload (mu)
aggrstabName= workingDir+'aggrstab.map'    # aggregate stability number (-)

# channel maps
lddchanName= workingDir+'lddchan.map'      # channel 1D network
chanwidthName= workingDir+'chanwidt.map'   # channel width (m)
...
chanleveesName= workingDir+'chanlevee.map' # main levees along channels
chanksatName= workingDir+'chanksat.map'    # ksat in case channel infiltrates, for dry channels
baseflowName= workingDir+'baseflow.map'    # stationary baseflow at end piints of river
...
if __name__ == "__main__":
    ...
    #workingDir = 'C:/CRCLisem/maps/'      #  folder with PCRaster base maps
    #inputDir = 'C:/CRCLisem/base/'

    os.chdir(inputDir)

    print(">>> read base maps from {0}".format(inputDir),flush=True)
