...
#from pyClassDemDerivatives import DEMderivatives
...
class ChannelMaps(StaticModel):
    ...
    def initial(self):
        ...
        chanman = cover(chanmask*chN,0)*mask
        ...
        chanwidth = lenrel*(chWidthmap-chWidthS)+chWidthS
        ...
        chandepth = lenrel*(chDepthmap-chDepthS)+chDepthS
...
class InfrastructureMaps(StaticModel):
    ...
    # ESPG = int(ESPGnumber)
        shape_v = ogr.Open(shpName)
        shape_l = shape_v.GetLayer()
        output = nametif
        x_res = dx/5.0
        y_res = x_res
        target_ds = gdal.GetDriverByName('GTiff').Create(output, x_res, y_res, 1, gdal.GDT_Byte)
        target_ds.SetGeoTransform((llx, x_res, 0, lly, 0, x_res))
        ...
        # resample tif to original dx with cubic
...
if __name__ == "__main__":
   ...
   #    sys.exit()
    ...
    # fill in missing values and convert to correct SI units for pedotransfer
