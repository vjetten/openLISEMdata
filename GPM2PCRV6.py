...
print(">>> nr GTiff files to be processed: {0}  ".format(len(hdflinks)),flush = True)
...
if optionGaugeGPM == 1:
    ...
    nr = 0
    update_progress(0)
    raintxtname = outputdir+pointnameIn
    with open(raintxtname, 'w') as f:
        f.write('# GPM point data \n')
        f.write('2\n')
        f.write('time (ddd:mmmm)\n')
